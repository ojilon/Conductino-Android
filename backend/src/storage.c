 ...
 * Requires third_party/sqlite/sqlite3.{c,h} (amalgamation you provide). */
...
int aurora_cache_check(const char *url, char *out_path, size_t max_len) {
    ...
    if (sqlite3_prepare_v2(g_db, sql, -1, &stmt, 0) == SQLITE_OK) {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            time_t saved_time = (time_t)sqlite3_column_int64(stmt, 1);
...
void aurora_cache_save(const char *url, const char *local_path) {
    if (!g_db) return;
    const char *sql =
        "INSERT OR REPLACE INTO page_cache(url, local_path, timestamp) VALUES(?, ?, ?);";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(g_db, sql, -1, &stmt, 0) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, url, -1, SQLITE_STATIC);
